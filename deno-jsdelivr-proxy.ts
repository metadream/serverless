...
const upstream = "https://cdn.jsdelivr.net/gh/seatwork";

serve(async (req: Request): Promise<Response> => {
  let path = (new URL(req.url)).pathname;
  if (path.startsWith("/gh/seatwork/")) {
    path = path.replace("/gh/seatwork/", "/");
  }
  if (path.startsWith("/lib")) {
    path = path.replace("/lib", "/front-end-libs");
  }

  switch (path) {
    case "/":
      return new Response("Upstream: https://cdn.jsdelivr.net");
    case "/favicon.ico":
    ...
    default:
      return await fetch(new Request(upstream + path), req);
  }
});